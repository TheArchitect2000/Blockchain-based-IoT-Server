...
const DeviceTable = memo(
    ({
        ...
        const [selectShareDialog, setSelectShareDialog] =
            useState<boolean>(false)
        const [globalShareDialog, setGlobalShareDialog] =
            useState<boolean>(false)
        ...
        const [sharedUsersDialog, setSharedUsersDialog] =
            useState<boolean>(false)
        ...
        async function handleDeviceRename() {
            if (newDeviceName.length === 0) {
                return toast.push(
                    <Notification
                        title={
                            'The device name must contain at least one letter.'
                        }
                        type="danger"
                    />,
                    {
        ...
        const columns = useMemo<ColumnDef<DeviceData>[]>(
            () => [
                {
                    ...
                    cell: (props) => {
                        ...
                        return (
                            <DeviceProductColumn
                                dataReceived={
                                    payloads[row.deviceEncryptedId]?.received
                                }
                                row={row}
                            />
                        )
            ...
            [payloads]
        ) // Only recreate if payloads changes
                        ...
                        {type === 'all' && (
                            <>
                                <p className="text-center text-lg">
                                    To add your device to this account, please
                                    use the FidesInnova mobile app.
                                </p>
                                <a
