...
const Statistic = ({ data = [] }: { data?: Partial<Statistic>[] }) => {
    ...
    const totalInstallations =
        services?.data?.data.reduce(
            (acc, item) => acc + item.numberOfInstallations,
            0
        ) ?? 0

    const totalInstallationPrice =
        services?.data?.data.reduce(
            (acc, item) => acc + item.installationPrice,
            0
        ) ?? 0

    const totalRunningPrice =
        services?.data?.data.reduce(
            (acc, item) => acc + item.runningPrice,
            0
        ) ?? 0
