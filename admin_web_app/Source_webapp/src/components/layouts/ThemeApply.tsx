...
function findClosestTailwindColor(hex: string): string {
    const rgb = hexToRgb(hex)
    let closestColor = ''
    let minDistance = Infinity

    for (const [colorName, colorHex] of Object.entries(tailwindColors)) {
        const tailwindRgb = hexToRgb(colorHex)
...
export default function ThemApply() {
    ...
    useEffect(() => {
        async function getServerTheme() {
            ...
            const colorSplited = findClosestTailwindColor(
                '#' + data.button
            ).split('-')
