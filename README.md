...
```
This command generates `fullchain.pem` and `privkey.pem` in either `/etc/letsencrypt/admin.YOURDOMAIN.COM` or `/etc/letsencrypt/panel.YOURDOMAIN.COM`. Next, create the `ssl` folder inside `/etc/nginx` and copy both `fullchain.pem` and `privkey.pem` into `/etc/nginx/ssl`.
